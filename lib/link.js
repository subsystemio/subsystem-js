...
class Link {
  constructor(ipc, opts = {}) {
    ...
    this.admins = (opts.admins || []).map((a) =>
      typeof a === 'string' ? b4a.from(a.trim(), 'hex') : a
    )
    ...
    ipc.onReport = (state) =>
      this._broadcast('sendStateReport', { state: JSON.stringify(state ?? null), ts: now() })
    ipc.onEvent = (name, payload) =>
      this._broadcast('sendEvent', { name, payload: JSON.stringify(payload ?? null), ts: now() })
  ...
  _onConnection(socket) {
    ...
    const channels = createChannels(socket, {
      ...
      onCommand: (m) => {
        if (gate.verified) this._onCommand(m, peer, tag)
      }
    })
    ...
    socket.on('close', () => {
      ...
      this.log(
        (peer.admin ? 'admin ' : 'observer ') + tag + ' detached (' + this.peers.size + ' watching)'
      )
    })
  }
