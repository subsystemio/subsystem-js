...
class UIHost {
  ...
  setAssets (dir) { this.assets = dir }

  broadcast (obj) {
    const data = JSON.stringify(obj)
    for (const ws of this.clients) { try { ws.write(data) } catch { /* dropped */ } }
  }
  ...
  _route (req, res) {
    ...
    if (this.assets && url.startsWith('/media/')) return this._serveAsset(req, res, path.basename(url))
  ...
  async stop () {
    for (const ws of this.clients) { try { ws.end() } catch {} }
    this.clients.clear()
