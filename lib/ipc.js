...
class IPC {
  ...
  serveUI (html) { this.ui.setHtml(html) }
  sendUI (msg) { this.ui.broadcast(msg) }
  onUIMessage (fn) { this._onUIMessage = fn }

  describe (caps) { this.caps = caps; this.onDescribe(caps) }
  ...
  onSetState (fn) { this._onSetState = fn }
  onCommand (fn) { this._onCommand = fn }

  setState (phase) { this._onSetState(phase) }
  command (name, args) { return this._onCommand(name, args) }
}
