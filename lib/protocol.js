...
const capability = {
  preencode (state, m) { cenc.fixed32.preencode(state, m.proof) },
  encode (state, m) { cenc.fixed32.encode(state, m.proof) },
  decode (state) { return { proof: cenc.fixed32.decode(state) } }
}

// hello { fwVersion } — sent on connect. No propId; identity is the connection's public key.
const hello = {
  preencode (state, m) { cenc.string.preencode(state, m.fwVersion) },
  encode (state, m) { cenc.string.encode(state, m.fwVersion) },
  decode (state) { return { fwVersion: cenc.string.decode(state) } }
}
